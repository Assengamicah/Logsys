 ...
 */
class Orderitemsb extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
	...
	{
	  $data = [];
	  $rst = Yii::$app->db->createCommand("SELECT CONCAT(gid,':',cper),CONCAT(name,' - ',cper) FROM itemgroup WHERE cper = 'BUNDLE' ORDER BY name")->queryAll(false);
	  foreach($rst as $rs)
	  {
	    $data[$rs[0]] = $rs[1];
	  }
	...
	{
	  $data = [];
	   if (Yii::$app->request->post('Orderitemsb')['gid']) 
		{
			$pid = Yii::$app->request->post('Orderitemsb')['gid'];
			$exp = explode(":",$pid);
