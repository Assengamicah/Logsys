 ...
 */
class Orderitems2 extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
	...
	{
	  $data = [];
	  $rst = Yii::$app->db->createCommand("SELECT CONCAT(gid,':',cper),CONCAT(name,' - ',cper) FROM itemgroup WHERE cper = 'CBM' ORDER BY name")->queryAll(false);
	  foreach($rst as $rs)
	  {
	    $data[$rs[0]] = $rs[1];
	  }
